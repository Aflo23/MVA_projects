...
for epoch in range(epochs):
    ...
    for i in range(0, n_train, batch_size):
        ...
        ### batch is taking the indices, apply a between i and i+batch size you take all of these 

        # choisir des batch parmi le truc 
        
        batch_indices = idx[i:i+batch_size]
        x_batch = torch.tensor(X_train[batch_indices]).to(device)
        y_batch = torch.tensor(y_train[batch_indices]).to(device)
...
for epoch in range(epochs):
    ...
    for i in range(0, n_train, batch_size):
        ...
        # your code here #
        batch_indices = idx[i:i+batch_size]
        x_batch, y_batch = torch.tensor(X_train[batch_indices]).to(device), torch.tensor(y_train[batch_indices]).to(device)

        print(f'type of x_batch, y_batch LSTM {type(x_batch), type(y_batch)}')
        ##################

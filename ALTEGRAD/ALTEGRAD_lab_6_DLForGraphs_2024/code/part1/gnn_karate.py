...
features = np.ones((n, 1)) #np.random.randn(n, 16)  # your code here # First features = np.random.randn(n, 16) afterwards 
...
adj = torch.FloatTensor(adj).to(device)  # Convert to dense torch tensor
adj = adj.to_sparse()  # Convert it to a sparse tensor
...
# Testing
model.eval()
output, alpha = model(features, adj) # don't add the alpha
...
############## Task 4
alpha = alpha.detach().cpu().numpy() # your code here # ## we need to convert tensor into array 
